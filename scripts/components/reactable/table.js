...
export class Table extends React.Component {
    ...
    sortByCurrentSort() {
        ...
        console.log(currentSort.column)
    ...
    onSort(column) {
     ...
     let selectedRows = this.props.curSelect;
     if (Object.getOwnPropertyNames(selectedRows).length > 0) {
        alert('Starting ' + key.label + ' on ' + Object.getOwnPropertyNames(selectedRows).length + ' row(s). See console for object details.');
        console.log('Starting ' + key.label + ' on ' + Object.getOwnPropertyNames(selectedRows).length + ' row(s). See console for object details.');
        console.log('Push the following data objects together. Fake ajax basically goes here.');
        Object.keys(selectedRows).map(function(e){
            console.log(selectedRows[e]);
        });

        console.log('TODO: Push changes to server. Once success callback is fired, update the state.dataSet to reflect.')
       
     }
     else {
        alert('ERROR!: You must select at least 1 row');
     }
    ...
    render() {
        ...
        let firstChild = null;
        let isItSelected = this.state.selected ? 'selected' : '';
        ...
        let totalItems = filteredChildren.length;
        ...
        return (
            ...
            <Dropdown options={options} onChange={this.onSelect.bind(this)} placeholder="Bulk Action" />
            <input placeholder="Search" onChange={this.props.getFilterString} />
            <Paginator {...props} colSpan={columns.length}
